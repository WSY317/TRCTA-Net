...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    # 所有子类都应该覆盖:
    #
    # -方法: ' load_annotations '，支持从从注释文件下载信息。
    # -方法: ' prepare_train_frames '，提供训练数据。
    # 方法: ' prepare_test_frames '，提供测试数据。
    #
    # 参数:
    # ann_file(str): 注释文件的路径。
    # pipeline(list[dict | callable]): 一个数据转换序列。
    # data_prefix(str): 存放视频的目录路径。默认值:”。
    # test_mode(bool): 在构建测试或验证数据集时存储True。默认值: False。
    # multi_class(bool): 确定数据集是否是多类数据集。默认值: False。
    # num_classes(int | None): 数据集的类数，用于多类数据集。默认值: 没有。
    # start_index(int): 考虑到不同的文件名格式，指定帧的开始索引。然而,
    # 如果将视频作为输入，则应将其设置为0，因为从视频加载的帧从0开始计数。默认值: 1。
    # modality(str): 数据的模态。支持“RGB”，“流”，“音频”。默认值:“RGB”。
    # memcached(bool): 是否在memcached中缓存关键点。如果设置为True，将使用
    # 'frame_dir'
    # 作为键
    # 从memcached中获取
    # 'keypoint'。默认值: False。
    # mc_cfg(元组): memcached客户端的配置，仅当
    # ' memcached==True '
    # 时适用。
    # 默认值: ('localhost'， 22077)。
    ...
    def __init__(self,
        ...
        self.pipeline = Compose(pipeline)#这就是第一部分实例化了一个图像增强的类。
    ...
    def prepare_train_frames(self, idx):
        ...
        return self.pipeline(results)#进入管道前的result
    ...
    def __getitem__(self, idx):
        ...
        #首先借助idx读取data，然后利用prepare_train_img完成data的图像增强。之后，return data。
