...
class UniformSampleFrames:
    ...
    从视频中采样一个n帧的片段。UniformSampleFrames基本上
    将视频分成n个等长的片段，随机抽取一个
    每个片段中的帧。为了使测试结果可重复，
    在测试时设置随机种子，使采样结果确定的。

    必需的键是“total_frames”，“start_index”，添加或修改的键
    分别是frame_indds, clip_len, frame_interval和num_clips。

    参数:
    clip_len(int): 每个采样输出剪辑的帧数。
    num_clips(int): 要采样的剪辑数量。默认值: 1。
    seed(int): 测试期间使用的随机种子。默认值: 255。"""
