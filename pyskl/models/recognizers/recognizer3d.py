...
class Recognizer3D(BaseRecognizer):
    ...
    def forward_train(self, imgs, label, **kwargs):
        ...
        x = self.extract_feat(imgs)#2 17 288 7 16  x 4 512 24 7 7

        cls_score = self.cls_head(x)#4 60
        gt_label = label.squeeze()#4 [57 51 32 1]
        loss_cls = self.cls_head.loss(cls_score, gt_label, **kwargs)#top1 top5
